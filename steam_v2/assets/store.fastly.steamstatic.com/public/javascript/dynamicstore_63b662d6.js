...
GDynamicStore = {
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		...
		else if ( Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle )
		{
			...
			$Description.append( '<div>购买该捆绑包即可为您未拥有的 %2$s 件物品省 %1$s%%！</div>'.replace( '%1$s', Math.round( Bundle.m_nDiscountPct ) ).replace( '%2$s', Bundle.m_cUserItemsInBundle ).replace( '%%', '%' ) );
		...
		else
		{
			...
			{
				$Description.html( '<div>购买此捆绑包，即可获得所有 %1$s 项内容！</div>'.replace( '%1$s', Bundle.m_cTotalItemsInBundle ) );
			}
			else {
				$Description.html( '<div>购买此捆绑包，所有 %2$s 个项目立省 %1$s%%！</div>'.replace( '%1$s', Math.round( Bundle.m_nDiscountPct ) ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ).replace( '%%', '%' ) );
			}
		}
...
GStoreItemData = {
	...
	BuildSupportedPlatformIcon: function( rgItemData )
	{
		...
		if ( rgItemData.virtual_reality )
		{
			...
			strHTML += '<span class="vr_required">VR 独占</span>';
		}
...
function UpdateStoreBannerForAdditionalCartDiscount( nCartDiscount )
{
	...
	<div class="placeHolder_lunarSale2019_giftActiveBar">	\
		...
		<div class="lunar_sale_title"><img src="https://store.fastly.steamstatic.com/public/images/promo/lunar2019/lny2019_title_zh-cn.png"/></div> \
...
function BuildCreatorCapsuleToAppend( oItem )
{
	...
	$FollowLink.bind('updateFollowState', function () {
		...
		{
			$FollowLink.html( '<span><img src="https://store.fastly.steamstatic.com/public/images/v6/ico/ico_selected_green.png">已关注</span>' );
